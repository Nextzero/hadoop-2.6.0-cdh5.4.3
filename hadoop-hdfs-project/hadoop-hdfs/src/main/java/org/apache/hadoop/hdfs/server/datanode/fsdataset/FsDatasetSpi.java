...
public interface FsDatasetSpi<V extends FsVolumeSpi> extends FSDatasetMBean {
  ...
  public void addVolume(
    ...
    /*
    czhc: 为什么是List呢？ 在DN上，每个bpid/namespace会对应一个bpos,而每个bpos在注册之后
    能获取到一个NamespaceInfo，因此才会有一个List<NamespaceInfo>
    拿到list后，要在volume目录下创建对应的BP目录
    */
  ...
  //czhc: DatanodeStorage对应DN下配置的一个目录,  通过观察DN配置的目录可知，配置的不同目录，VERSION里面的storageId是不一样的。

  /** @return one or more storage reports for attached volumes. */
  public StorageReport[] getStorageReports(String bpid)
      throws IOException;
    /*
        StorageReport仅包含一个Volume的使用量统计值，如已使用多少等。
        一个Volume对应一个StorageReport，因此为数组结构
     */
